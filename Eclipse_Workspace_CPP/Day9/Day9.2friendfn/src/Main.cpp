...
// If we want to access private and protecyted membeers of class inside
//non member function of class then
//Make non member function as a friend function inside class

/*
 friend, global, and static member function do not gets this pointer
 in this case must declare in class only
 Complex sum(Complex &other){
 Complex result;
 result.real = this->real + other.real;
 result.img = this->img + other.img;
